...
Este cap\'itulo detalla la formulaci\'on matem\'atica y f\'isica que fundamenta la transici\'on tecnol\'ogica desde la refrigeraci\'on convencional por aire hacia los sistemas de termosif\'on y cambio de fase para disipar altas densidades de potencia \cite{incropera2007}.

\section{El muro t\'ermico y los l\'imites del calor sensible}\label{sec:calor_sensible}
...
\begin{equation}\label{eq:calor_sensible}
...
Considerando un rack est\'andar de IA de $100\text{ kW}$ ($\dot{Q} = 100.000\text{ W}$), con una elevaci\'on m\'axima de temperatura de aire permitida de $15^\circ\text{C}$ ($\Delta T = 15\text{ K}$) y un calor espec\'ifico del aire a nivel del mar ($C_p \approx 1.006\text{ J}/(\text{kg}\cdot\text{K})$), el caudal m\'asico requerido es \cite{ashrae2021}:
\begin{equation}
\dot{m}_{\text{aire}} = \frac{\dot{Q}}{C_p \cdot \Delta T} = \frac{100.000}{1.006 \cdot 15} \approx 6,627\text{ kg/s}
\end{equation}
Dada la densidad del aire a $20^\circ\text{C}$ ($\rho_{\text{aire}} \approx 1,2\text{ kg/m}^3$), el caudal volum\'etrico ($\dot{V}_{\text{aire}}$) equivalente es \cite{incropera2007}:
\begin{equation}
\dot{V}_{\text{aire}} = \frac{\dot{m}_{\text{aire}}}{\rho_{\text{aire}}} = \frac{6,627}{1,2} \approx 5,52\text{ m}^3\text{/s} = 5.520\text{ L/s}
\end{equation}
Mover $5.520\text{ Litros de aire por segundo}$ a trav\'es de un espacio confinado de rack de 19 pulgadas genera p\'erdidas de carga (ca\'ida de presi\'on) exponenciales y requiere ventiladores gigantescos cuyo consumo el\'ectrico neutraliza la eficiencia operativa del centro de datos \cite{uptime2024}.

\section{Termodin\'amica del cambio de fase (calor latente)}\label{sec:calor_latente}
...
\end{equation}
Al comparar ambos m\'etodos, el cambio de fase permite una reducci\'on del caudal volum\'etrico requerido de aproximadamente \textbf{9.200 veces} frente al aire \cite{webb2005}.

\section{M\'etodo \texorpdfstring{$\epsilon$}{epsilon}-NTU para intercambiadores de calor bif\'asicos}\label{sec:metodo_ntu}
...
\begin{equation}\label{eq:efectividad_ntu}
...
\begin{equation}\label{eq:efectividad_cambio_fase}
\epsilon = 1 - \exp(-NTU)
\end{equation}
...
\section{Algoritmo de PUE din\'amico en funci\'on del clima}\label{sec:algoritmo_pue}
El PUE total del sistema se modela en funci\'on de la temperatura del aire exterior ($T_{ext}$) y la temperatura cr\'itica ($T_{cr}$) para Free Cooling total \cite{greengrid2008}. El algoritmo opera bajo tres estados:
\begin{enumerate}
    ...
    \end{equation}
\end{enumerate}
Este modelo din\'amico, alimentado por datos meteorol\'ogicos horariales, es el que permite calcular el ahorro OPEX y determinar la viabilidad t\'ermica \cite{greenberg2006}.
