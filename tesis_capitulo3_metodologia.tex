...
\section{Arquitectura del simulador (Python)}\label{sec:arquitectura_simulador}
El modelamiento del comportamiento t\'ermico y de costos operativos se program\'o en el entorno local utilizando Python 3.11, estructurado en dos scripts principales \cite{incropera2007}:
\begin{enumerate}
...
La simulaci\'on se configur\'o con par\'ametros constantes representativos de la demanda de superc\'omputo de Inteligencia Artificial para el a\~no 2026 \cite{ashrae2021}:
\begin{itemize}
    \item Carga T\'ermica de TI ($P_{IT}$): $100\text{ kW}$ constantes ($24/7/365$).
    \item Rango de Operaci\'on del Procesador: Temperatura de uni\'on m\'axima permitida en el silicio ($T_{j,max}$) de $75^\circ\text{C}$ para evitar estrangulamiento t\'ermico \cite{ashrae2021}.
    \item Flujo Volum\'etrico de Fluidos Diel\'ectricos: Configurado en $\dot{V}_{\text{bifasico}} = 0,6\text{ L/s}$ para ebullici\'on saturada (ej. 3M Novec 7100), logrando una reducci\'on volum\'etrica de $9.200$ veces en comparaci\'on con el caudal de aire requerido ($5.520\text{ L/s}$) para disipar la misma carga \cite{garrity2014}.
\end{itemize}

\section{Modelamiento del PUE din\'amico}\label{sec:modelamiento_pue_dinamico}
El c\'alculo del PUE din\'amico de enfriamiento se rige por el algoritmo termodin\'amico detallado en la Sec.~\ref{sec:algoritmo_pue}, evaluando la relaci\'on entre la temperatura del aire exterior ($T_{ext}$) y la temperatura cr\'itica de transici\'on ($T_{cr}$) definida \cite{greengrid2008} para cada tecnolog\'ia:
\begin{itemize}
...
\end{tikzpicture}
\caption{Diagrama esquem\'atico del sistema de inmersi\'on l\'iquida bif\'asica (Fuente: Elaboraci\'on propia).}
\label{fig:immersion_diagram}
\end{figure}


\section{Pipeline de datos e ingesta en GCP}\label{sec:pipeline_gcp}
Para procesar de forma masiva los datos meteorol\'ogicos de 39 localidades geogr\'aficas ($26.280\text{ horas}$ por ubicaci\'on, totalizando $1.025.856$ registros de temperatura), se dise\~n\'o \cite{masnet2020} un flujo de trabajo ETL (Extract, Translate, Load) automatizado:
\begin{enumerate}
    ...
    \item \textbf{Ingesta incremental:} Script \texttt{daily\_weather\_etl.py} que consulta de manera diaria el clima del d\'ia anterior de las 39 localidades del dataset y lo sube a la tabla.
\end{enumerate}

Para asegurar la idempotencia del ETL ante posibles re-ejecuciones, el script ejecuta una sentencia DML \texttt{DELETE} para limpiar los registros existentes de la fecha destino en BigQuery antes de cargar los nuevos datos \cite{google2023}.

\section{Esquema de base de datos en BigQuery}\label{sec:esquema_bigquery}
Los datos se estructuraron bajo dos tablas en BigQuery \cite{google2023}:
\begin{itemize}
